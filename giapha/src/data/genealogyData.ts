...
export const GENEALOGY_DATA: GenealogyMember[] = [
  {
    id: "1",
    code: "1",
    generation: "Đời 11",
...
export const GENEALOGY_SUMMARY_TEXT = GENEALOGY_DATA.map(
  (m) =>
    `[Mã ${m.code}] ${m.generation} - ${m.fullName}${
      m.otherName ? ` (${m.otherName})` : ''
    } | Trực hệ: ${m.relationship}${
      m.spouse ? ` | Vợ/Chồng: ${m.spouse}` : ''
    }${m.birthDeathInfo ? ` | Sinh/Mất/Mộ: ${m.birthDeathInfo}` : ''}${
      m.notes ? ` | Ghi chú/Con cái: ${m.notes}` : ''
    }`
).join('\n');
